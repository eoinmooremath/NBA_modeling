...
def main():
    ...
    model = SAINT(
        ...
        attention_dropout=saint_config['attention_dropout'],
        ffn_dropout=saint_config['ffn_dropout'],
        mlp_dropout=saint_config['mlp_dropout']
    ).to(device)
    ...
    torch.save({
        ...
        'config': config['model']['saint']  # Save the full model config
    }, save_path)
