...
class GameDataset(Dataset):
    ...
    def __init__(self, X, Y, cat_cols, num_players=10, continuous_mean_std=None):
        ...
        try:
            ...
            if continuous_mean_std is not None:
                mean_tensor = torch.tensor(self.train_mean, dtype=torch.float32)
                std_tensor = torch.tensor(self.train_std, dtype=torch.float32)
                std_tensor = torch.where(std_tensor < 1e-8, torch.ones_like(std_tensor), std_tensor)
                self.X_continuous = (self.X_continuous - mean_tensor) / std_tensor
