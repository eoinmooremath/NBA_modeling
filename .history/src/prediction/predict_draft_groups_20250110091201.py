...
def automate_predictions(model_path, config, n_games, m_players=6):
    ...
    for group in group_list:
        ...
        if not final_df.empty:
            ...
            draft_group_dic[draft_group] = result
...
if __name__ == "__main__":
    ...
    base_output_dir = Path(config['paths']['predictions']['output'])
    ...
    # Process and save each draft group
    for draft_group, df in results.items():
        ...
        print(df_sorted.round(2).to_string(index=False))
        print(f"\nTotal Salary: ${df_sorted['Salary'].sum():,}")
        print(f"Projected Points: {df_sorted['Player Points'].sum():.2f}")
        print("-" * 80)
        
        # Save as CSV in date subfolder
        csv_file = date_dir / f'draft_group_{draft_group}_{timestamp}.csv'
        df_sorted.to_csv(csv_file, index=False)
    
    print(f"\nCSV files have been saved to: {date_dir}")
...
#             # Save draft groups
#             csv_file = f'draft_group_{draft_group}_time_{timestamp}.csv'
